...
/*
    ...
    void testCreateThreshold() {
        String organizationId = "ORG001";
        Threshold thresholdCreated = new Threshold("ORG001","ITEM001","LOCATION001",10,100);

        thresholdCreated.setId("THRESHOLD001");
        ThresholdService thresholdService = mock(ThresholdService.class);

        when(thresholdService.createThreshold(eq(organizationId), any(Threshold.class))).thenReturn(thresholdCreated);

        ResponseEntity<Threshold> response = thresholdController.createThreshold(organizationId, thresholdCreated);

        assertEquals(HttpStatus.CREATED, response.getStatusCode());
        assertEquals(thresholdCreated, response.getBody());

        verify(thresholdService, times(1)).createThreshold(organizationId, thresholdCreated);
        verifyNoMoreInteractions(thresholdService);
    }
    */
    ...
    void testDeleteThreshold() {
        ...
        assertEquals("Threshold is deleted!", response.getBody());
