...
class LocationServicesTest {
    ...
    public void testCreateLocation_WithValidOrganizationAndNonExistingLocation_ShouldCreateLocation() {
        ...
        // Assert
        assertNotNull(result);
        assertEquals("ORG001_111", result.getLocationId());
        assertEquals("locationDesc", result.getLocationDesc());
    ...
    @Test
    public void testCreateLocation_WithInvalidOrganization_ShouldThrowOrganizationNotFoundException() {
        // Arrange
        when(organizationRepository.findById(anyString())).thenReturn(Optional.empty());

        // Act & Assert
        assertThrows(OrganizationNotFoundException.class, () -> locationService.createLocation("org1", new Location()));
        verify(organizationRepository, times(1)).findById(anyString());
        verify(locationRepository, never()).findByLocationIdAndOrganizationId(anyString(), anyString());
